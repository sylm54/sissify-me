...
## Structure
Hypno sessions must be built from small, reusable subscripts rather than one big script. Create separate subscripts for induction, deepening, redeepening, and for the different triggers or suggestions. The final session script should only contain the Pre-Talk, Post-hypnotic suggestions, Emergence, and Post-talk — the parts that change from file to file. Everything else should link to one or a collection of subscripts.

Keep the shared structural subscripts (induction, deepening, redeepening, emergence) in reusable files so every session links to the same ones, and keep the content subscripts (trigger blocks, suggestion pools) in their own files so they can be swapped and updated independently. Build subscripts with a subagents.
